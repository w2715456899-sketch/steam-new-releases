...
- **看某個標籤的遊戲**：點任何一款遊戲下面的標籤（例如「Rogue」），會列出目前紀錄裡所有有這個
  標籤的遊戲，不限日期。
- **好評率**：有累積到足夠評論的遊戲，會顯示跟 Steam 官方頁面一樣的評價文字（例如「壓倒性好評
  96%」），顏色跟著好評/褒貶不一/差評分三色；太新、評論數不夠的遊戲不會顯示（Steam 自己也還沒
  給分）。
- **分享給朋友**：把上面那個網站網址傳給他們就好，不需要帳號、不需要裝東西；貼到 Discord/LINE
  會自動顯示標題、簡介跟封面圖預覽卡片（Open Graph）。
- **Discord 通知**：每天早上 6:00（台北時間）先收到一則「日期 新遊戲來了 🫠 共 N 款」+ 網站
  連結的文字訊息，接著每 10 款遊戲一批，用 Discord embed 附上封面縮圖、名稱、狀態（已上架／
  預計上架）跟價格，點連結可以直接跳到那款遊戲的 Steam 頁面；當天沒有新遊戲就不會發訊息
  （安靜不打擾）。
- **自動更新**：GitHub Actions 每小時執行一次（`.github/workflows/update.yml`），抓資料 →
  ...
  不用裝任何東西。
- **失敗告警**：如果某次執行中途出錯（API 掛掉、程式例外、git push 失敗…），會額外發一則
  Discord 訊息「⚠️ Steam 新遊戲更新失敗」+ 該次執行的 Actions 紀錄連結，不會悄悄失敗都沒人
  發現。
